...
def iniciar_base_datos():
    ...
    cursor = conn.cursor()
    # Tabla para apuestas del bot (over 0.5 goles)
    cursor.execute('''
    ...
    ''')
    # Tabla para apuestas en vivo
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS apuestas_vivo (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            fecha TEXT,
            bankroll REAL,
            cuota REAL,
...
def calcular_apuesta(bankroll, cuota, prob_exito, kelly_frac=0.25, apuesta_max_frac=0.1, apuesta_min=1):
    b = cuota - 1
    p = prob_exito
    q = 1 - p
    if b <= 0:
    ...
    <body>
        <div class="container mt-4">
            ...
            <div class="card p-4">
                ...
                <a href="/bot" class="btn btn-primary w-100 mb-3">Bot Over 0.5 Goles</a>
                <a href="/vivo" class="btn btn-primary w-100">Apuestas en Vivo</a>
            </div>
        </div>
...
# Página de apuestas del bot (over 0.5 goles)
...
def bot():
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Bot Over 0.5 Goles</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
        <style>
            body { background-color: #f0f4f8; }
            .container { max-width: 600px; }
    ...
    <body>
        <div class="container mt-4">
            <h1 class="text-center mb-4">Bot Over 0.5 Goles</h1>
            <div class="card p-4">
                <form method="POST">
                    <div class="mb-3">
                ...
                {% endif %}
                <a href="/historial_bot" class="btn btn-secondary w-100 mt-3">Ver Historial</a>
                <a href="/" class="btn btn-secondary w-100 mt-3">Volver</a>
            </div>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    </body>
...
def vivo():
    ...
    if request.method == 'POST':
        try:
            ...
            if bankroll <= 0 or cuota <= 1 or not (0 <= prob_exito <= 1):
                ...
                fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                cursor.execute('''
                    INSERT INTO apuestas_vivo (fecha, bankroll, cuota, precision, apuesta, ganancia_potencial, resultado, nuevo_bankroll)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (fecha, bankroll, cuota, prob_exito, apuesta, ganancia, "pendiente", bankroll))
                conn.commit()
                apuesta_id = cursor.lastrowid
    ...
    <body>
        <div class="container mt-4">
            <h1 class="text-center mb-4">Apuestas en Vivo</h1>
            <div class="card p-4">
                <form method="POST">
                    <div class="mb-3">
                        <label for="bankroll" class="form-label">Bankroll ($):</label>
                    ...
                    <div class="mb-3">
                        <label for="cuota" class="form-label">Cuota (ej. 1.50):</label>
                        <input type="number" step="0.01" class="form-control" id="cuota" name="cuota" required>
                    </div>
                    <div class="mb-3">
                        <label for="precision" class="form-label">Precisión (0-1):</label>
                        <input type="number" step="0.01" class="form-control" id="precision" name="precision" value="0.50" required>
                    </div>
                    <button type="submit" class="btn btn-primary w-100">Calcular Apuesta</button>
                </form>
                {% if error %}
                ...
                {% endif %}
                <a href="/historial_vivo" class="btn btn-secondary w-100 mt-3">Ver Historial</a>
                <a href="/" class="btn btn-secondary w-100 mt-3">Volver</a>
            </div>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
...
def historial_bot():
    ...
    <body>
        <div class="container mt-4">
            <h1 class="text-center mb-4">Historial Bot Over 0.5 Goles</h1>
                ...
                <tbody>
                    {% for apuesta in apuestas %}
                        <tr>
                            ...
                            <td>${{ "%.2f" % apuesta[6] }}</td>
                            <td>
                                <select class="form-select resultado" data-id="{{ apuesta[0] }}" data-tipo="bot">
                            ...
                            <td>${{ "%.2f" % apuesta[8] }}</td>
                            <td>
                                <a href="/eliminar_apuesta/bot/{{ apuesta[0] }}" class="btn btn-danger btn-sm" onclick="return confirm('¿Eliminar esta apuesta?')">Eliminar</a>
                            </td>
                        </tr>
                    {% endfor %}
                </tbody>
            </table>
            <a href="/bot" class="btn btn-primary w-100 mt-3">Volver</a>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
        <script>
            document.querySelectorAll('.resultado').forEach(select => {
                select.addEventListener('change', function() {
                    ...
                    fetch('/actualizar_resultado/' + tipo, {
                        method: 'POST',
...
# Historial de apuestas en vivo
@app.route('/historial_vivo')
def historial_vivo():
    conn = sqlite3.connect('apuestas.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM apuestas_vivo")
    apuestas = cursor.fetchall()
    conn.close()
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Historial Apuestas en Vivo</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
        <style>
            body { background-color: #f0f4f8; }
            .container { max-width: 1000px; }
    ...
    <body>
        <div class="container mt-4">
            <h1 class="text-center mb-4">Historial Apuestas en Vivo</h1>
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>ID</th>
                ...
                <tbody>
                    {% for apuesta in apuestas %}
                        <tr>
                            ...
                            <td>${{ "%.2f" % apuesta[6] }}</td>
                            <td>
                                <select class="form-select resultado" data-id="{{ apuesta[0] }}" data-tipo="vivo">
                                    <option value="pendiente" {% if apuesta[7] == "pendiente" %}selected{% endif %}>Pendiente</option>
                                    <option value="ganada" {% if apuesta[7] == "ganada" %}selected{% endif %}>Ganada</option>
                                    <option value="perdida" {% if apuesta[7] == "perdida" %}selected{% endif %}>Perdida</option>
                                </select>
                            ...
                            <td>${{ "%.2f" % apuesta[8] }}</td>
                            <td>
                                <a href="/eliminar_apuesta/vivo/{{ apuesta[0] }}" class="btn btn-danger btn-sm" onclick="return confirm('¿Eliminar esta apuesta?')">Eliminar</a>
                            </td>
                        </tr>
                    {% endfor %}
                </tbody>
            </table>
            <a href="/vivo" class="btn btn-primary w-100 mt-3">Volver</a>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
        <script>
            document.querySelectorAll('.resultado').forEach(select => {
...
@app.route('/actualizar_resultado/<tipo>', methods=['POST'])
def actualizar_resultado(tipo):
    data = request.get_json()
    apuesta_id = data['id']
    resultado = data['resultado']
    if resultado not in ["pendiente", "ganada", "perdida"]:
        return jsonify({'error': "Resultado inválido"}), 400
    tabla = 'apuestas' if tipo == 'bot' else 'apuestas_vivo'
    conn = sqlite3.connect('apuestas.db')
    cursor = conn.cursor()
    cursor.execute(f"SELECT bankroll, apuesta, ganancia_potencial FROM {tabla} WHERE id = ?", (apuesta_id,))
    ...
    cursor.execute(f'''
        UPDATE {tabla} SET resultado = ?, nuevo_bankroll = ?
        WHERE id = ?
...
# Eliminar apuesta
@app.route('/eliminar_apuesta/<tipo>/<int:id>')
def eliminar_apuesta(tipo, id):
    tabla = 'apuestas' if tipo == 'bot' else 'apuestas_vivo'
    conn = sqlite3.connect('apuestas.db')
    cursor = conn.cursor()
    ...
    conn.commit()
    conn.close()
    return redirect('/historial_bot' if tipo == 'bot' else '/historial_vivo')
